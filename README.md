...
The public site and admin panel use Supabase only. Google Sheets is only a private backup/import workflow, not a deployed website data source.
...
## Google Sheets Backup Import

Supabase is the runtime source of truth, but you can keep a private Google Sheet backup using the latest-standard CSV tabs generated by `npm run export:supabase-to-sheets`. If you later edit that backup sheet and want to import it back into Supabase, publish/export each tab as CSV and keep these local-only variables in `.env` temporarily:
...
Use the Supabase `service_role` key or a server-only Supabase secret key only on your own machine for this import. Never put it in browser code, Netlify frontend variables, GitHub, or chat.
...
Restore one deleted case from the backup sheet without touching testimonials or every other case:

```bash
npm run import:sheets-to-supabase -- --case HUM-001 --write
```

The importer expects the new Google Sheets backup with tabs named `Case Ledger`, `Case Images`, `Mentorship Testimonials`, `Support Categories`, and `Fund Types`. Previous sheet column names are no longer supported.

After the import succeeds, remove the sheet CSV URLs and service key from `.env`.
