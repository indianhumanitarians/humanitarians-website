...
const HomeHeroStats = () => {
  ...
  return (
    <div className="hero-card-stack">
      ...
      <p className={`hero-source-pill ${source}`}>{source === "live" ? "Live public stats" : source === "partial" ? "Live data partial" : "Live data unavailable"}</p>
    </div>
  );
};
...
const HomeImpactSnapshot = () => {
  ...
  const totalFund = zakatAmount + sadaqahAmount;
  const zakatPercent = totalFund > 0 ? (zakatAmount / totalFund) * 100 : 0;
  const sadaqahPercent = totalFund > 0 ? (sadaqahAmount / totalFund) * 100 : 0;
      ...
      <div className="impact-heading">
        <div className="impact-title-line">
          ...
          <p className={`impact-source-pill ${source}`}>
            {source === "live" ? "Live stats" : source === "partial" ? "Live data partial" : "Live data unavailable"}
          </p>
        </div>
        <h2 className="h2">Numbers that <em>mean something.</em></h2>
        <p className="lead">Public signals before the details - community size, case tracking, and how funds were used. Updated monthly.</p>
      </div>
      <div className="impact-top">
        {!hasStats ? <p className="empty-state">Live public stats are not available right now.</p> : null}
        <div>
        <div className="impact-stats">
        {metrics.map((item) => (
          <article className={`istat ${item.variant ?? ""}`} key={item.label}>
        ...
        <div className="fund-bar-card">
          ...
          <div className="fund-bar-label">
            <span>Zakat - {formatRupees(zakatAmount)}</span>
            <span>{zakatPercent.toFixed(1)}%</span>
          </div>
          <div className="fund-bar"><i className="fund-fill-g" style={{ width: `${zakatPercent}%` }} /></div>
          <div className="fund-bar-label">
            <span>Sadaqah - {formatRupees(sadaqahAmount)}</span>
            <span>{sadaqahPercent.toFixed(1)}%</span>
          </div>
          <div className="fund-bar"><i className="fund-fill-gold" style={{ width: `${sadaqahPercent}%` }} /></div>
        </div>
      </div>
      <article className="chart-card home-impact-chart">
        ...
        <div className="chart-wrap" aria-hidden="true">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={stats.monthly.filter((row) => row.total_cases > 0)} margin={{ top: 12, right: 6, left: -22, bottom: 0 }}>
              <CartesianGrid vertical={false} stroke="#efe7db" />
              <XAxis dataKey="period_label" tick={{ fontSize: 10, fill: "#8a8178" }} interval="preserveStartEnd" />
              <YAxis allowDecimals={false} tick={{ fontSize: 10, fill: "#8a8178" }} />
              <Tooltip cursor={{ fill: "rgba(26,92,56,0.08)" }} />
              <Bar dataKey="total_cases" name="Families helped" fill="#1a5c38" radius={[5, 5, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
  ...
  return (
    ...
    <section className="hero">
      <div className="hero-left">
        <div className="eyebrow"><span className="eyebrow-bar" />Community-driven giving</div>
        <h1 className="hero-h1">
          From support<br />to <em>self-reliance.</em>
        </h1>
        <p className="hero-p">
          A charity community supporting verified Zakat and Sadaqah cases through livelihoods, skills, and mentorship - helping families stand on their own feet.
        </p>
    ...
    <main>
      <section className="section stories-section">
        <div className="s-inner">
          ...
          <h2 className="h2">Real support. <em>Real turning points.</em></h2>
          <p className="lead">See the support - not private details. Every case is verified, anonymized, and reported publicly with dignity.</p>
          {storiesLoading ? <p className="soft-status">Loading public case stories...</p> : null}
          {featuredStories.length > 0 ? (
            <>
              ...
              <p className="stories-cta">
                <Button to="/case-stories" variant="primary">View all anonymized stories</Button>
              </p>
            </>
          ) : !storiesLoading ? (
            <p className="empty-state">No published case stories are available yet.</p>
          ) : null}
        </div>
      </section>
      ...
      <section className="section model-section">
        <div className="s-inner">
          ...
          <h2 className="h2">Built for <em>lasting impact,</em> not just relief.</h2>
          <p className="lead">Livelihood, skills, and mentorship for people to stand on their own feet - not just receive one-time aid.</p>
      ...
      <section className="section mentor-section">
        <div className="s-inner mentor-inner">
          <div>
            ...
            <h2 className="h2">Guidance that <em>changes directions.</em></h2>
            <p className="lead">Volunteers from IITs, colleges, and professional backgrounds help students and job seekers move with confidence - at no cost to them.</p>
            ...
            <div className="mentor-action">
              <Button to="/mentorship" variant="primary">Explore Mentorship</Button>
            </div>
          </div>
          <div className="mentor-visual">
            <blockquote className="mentor-quote">
              "The right guidance at the right moment can change a trajectory entirely."
            </blockquote>
            <p>
              Mentors help students choose the right courses, prepare for interviews, build communication skills, and find their first steps in a career. All mentorship is voluntary - from people who want to give back.
            </p>
      ...
      <section className="section transp-section">
        <div className="s-inner">
          ...
          <h2 className="h2">You deserve to know <em>where it goes.</em></h2>
          <p className="lead">Every rupee tracked. Every case anonymized. Every report published.</p>
          ...
          <div className="transp-action">
            <Button to="/reports" variant="primary">View monthly reports</Button>
          </div>
        </div>
      </section>
